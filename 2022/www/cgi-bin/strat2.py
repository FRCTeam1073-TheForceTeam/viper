    ...
    def update(self):
        ...
        for k in self.MatchStatsAgg.keys():
            try:
                ...
                self.MatchStatsAgg[k].Max = "--"
                self.MatchStatsAgg[k].Range = "--"
                self.MatchStatsAgg[k].n = "--"
                self.MatchStatsAgg[k].std = "--"

    def update2(self):
        self.RPAvg = round(np.mean(self.RPs), 2)
...
def _match_predictor():
    ...
    for i in range(len(matchSchedule)):
        ...
        try:
            ...
            #### Assigning Teams the RPs they earned.
            for x in [r1, r2, r3]:
                teams[x].RPs.append(RedRp)
            for x in [b1, b2, b3]:
                teams[x].RPs.append(BlueRp)
...
class page:
    ...
    def navBar(self):
        ...
        self.returnable += (
            ...
            + """<td><p onclick='window.location.href = "../../cgi-bin/strat.py";'>Refresh</p></td>"""
            + "</tr>\n"
        )
        self.spacing -= 1
    ...
    def teamList(self):
        ...
        self.returnable += (
            self.spacing * "\t"
        ) + "<tr><th>Team Number</th><th>Team Name</th><th>RPs</th></tr>\n"
    ...
    def RPteamList(self):
        self.returnable += (
            self.spacing * "\t"
        ) + "<div id='RPteamList' class='page' style='display:none'>\n"
        self.spacing += 1
        ...
        teamlists.sort()
        teamlists = [str(x) for x in teamlists]
        head = [["RP", "Team Name", "Team Number"]]
        result = []
        for t in teamlists:
            result.append([t, teams[t].teamName, teams[t].RPAvg])
        result = np.array(result)
        result: np.ndarray = result[result[:, 2].argsort()]
...
def redirect():
    print(
        """<!DOCTYPE html>\n<html lang='en'>\n<script>document.onload = window.location.href = "./../strat/index2.html";</script>"""
    )
...
if __name__ == "__main__":
    ...
    open("../html/strat/index2.html", "w").write(r)
