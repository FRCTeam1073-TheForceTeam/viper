...
if ("$event" eq "") {
	...
	# any elims?
	my %alliances;
	if ( -f "matchdata/$event.elims" ) {
	    $data = `cat matchdata/$event.elims`;
	    @lines = split /\n/, $data;
	    my $i = 1;
	    foreach my $line (@lines) {
		$alliances{$i} = $line;
	...
	for (my $m = 1; $m <= $count; $m++) {
		...
		print "<a href=\"${scout}?game=${key}&team=$mhash{$key}\">";
	...
	if (@all > 0) {
	    ...
	    my $semif = "matchdata/${event}.semis";
	    if (-f "$semif" ) {
		my @semit;
		if (open my $fh, "<", $semif) {
		    while (my $line = <$fh>) {
